...
from convnetpy.vol_util import augment
...
def load_data(training=True):
    ...
    imgs = [imgs[n:n + 784] for n in range(0, len(imgs), 784)]
    inputs = []
    V = Vol(28, 28, 1, 0.0)
    for img in imgs:
...
def start(conv):
    ...
    if conv:
        layers.append(
            {'type': 'conv', 'sx': 5, 'filters': 8, 'stride': 1, 'pad': 2, 'activation': 'relu', 'drop_prob': 0.5})
